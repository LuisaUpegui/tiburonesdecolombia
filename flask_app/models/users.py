...
class User:
    ...
    def save(cls, formulario):
        ...
        result = connectToMySQL('shark').query_db(query, formulario) #1 - Insert recibe id
    ...
    def update(cls, formulario): 
        ...
        result1 = connectToMySQL('shark').query_db(query, formulario)
        return result1
    ...
    def valida_usuario2(formulario2):
        ...
        query = "SELECT * FROM users WHERE email = %(email)s"
        results = connectToMySQL('shark').query_db(query, formulario2)
        if len(results) >= 1:
